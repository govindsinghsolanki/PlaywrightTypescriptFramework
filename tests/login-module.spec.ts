...
test("[Login] Verify that the user cannot login with an invalid password",{tag:['@Smoke','@Sanity','@InvalidPassword']},async({loginPage,credentialProvider,gotoUrl})=>{
    ...
    await expect(loginPage.getInvalidCredentialsErrorMessage()).toHaveText(loginModuleData.invalid_credentials_text,{timeout:10000});   
})
...
test("[Login] Verify that the user can log in with valid username and password",{tag:'@VISUAL'},async({gotoUrl,loginPage,credentialProvider,leftNavigationPage})=>{ 
    ...
    await expect(leftNavigationPage.orangeHrmLogoVisible()).toHaveScreenshot("OrangeHRMBrandLogo.png",{timeout:10000});
    await expect(leftNavigationPage.leftNavigationPanelVisible()).toHaveScreenshot("LeftNavigationPanel.png",{timeout:10000});
})